...
EXPERIMENT_NAME = os.getenv('MLFLOW_EXPERIMENT_NAME', 'svm_model')
...
class SVMTrainer:
    def __init__(self, test_size=0.2):
        ...
        self.model_config = {
            ...
            'params': {
                'classifier__C': [1.0, 10.0],  # Reduced options
                'classifier__kernel': ['rbf'],  # Only one kernel instead of multiple
                'classifier__gamma': ['scale'],  # Fixed value instead of multiple
    ...
    def train(self, X_train, X_test, y_train, y_test):
        """Train SVM model and track with MLflow"""
...
model_trainer = SVMTrainer()

@app.route('/health')
def health():
    return jsonify({
        "service": "svm_model_api",
        "status": "healthy"
    })
...
def download_model():
    ...
    try:
        ...
        return send_file(model_path, as_attachment=True, download_name='svm_model.joblib')
