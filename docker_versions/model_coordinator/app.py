...
@app.route('/preprocess_and_train', methods=['POST'])
def preprocess_and_train():
    """
    Perform full preprocessing pipeline and model training in a single orchestrated call.
    
    This endpoint accepts raw data and performs:
    1. Data cleaning
    2. Feature selection
    3. Anomaly detection
    4. Model training
    
    Returns consolidated results from all steps.
    """
    try:
        # Get data from request
        data = request.json
        logger.info(f"Received preprocess_and_train request with data keys: {data.keys()}")
        
        if not data or 'data' not in data or 'target_column' not in data:
            return jsonify({'error': 'Missing required data fields (data and/or target_column)'}), 400
        
        # Extract key parameters
        user_id = data.get('user_id')
        run_name = data.get('run_name', f"training_run_{time.strftime('%Y%m%d_%H%M%S')}")
        run_id = data.get('run_id')  # We may already have a run_id from the web app
        ...
        if not run_id:
            # Create training run in database if not already created
            run_id = create_training_run(user_id, run_name)
            if not run_id:
                return jsonify({'error': 'Failed to create training run in database'}), 500
            logger.info(f"Created new training run with ID {run_id} for user {user_id}")
        else:
            logger.info(f"Using existing training run with ID {run_id} for user {user_id}")
        
        # Get raw data in the right format
        raw_data = data['data']  # This should be list of records or dict of column arrays
        
        # Initialize result dictionary to collect all information
        result = {
            'run_id': run_id,
            'status': 'processing'
        }
        ...
        try:
            ...
            result['cleaner_config'] = {
                ...
                "encoding_mappings_summary": {col: len(mapping) for col, mapping in cleaning_result.get("encoding_mappings", {}).items()}
            }
            result['cleaning_report'] = cleaning_result.get("report", {})
            
            # Save large encoding mappings to a file if needed
            ...
            if encoding_mappings:
                try:
                    ...
                    mappings_dir = os.path.join('/app', 'static', 'temp', 'mappings')
                    os.makedirs(mappings_dir, exist_ok=True)
                    mappings_file = os.path.join(mappings_dir, f'encoding_mappings_{run_id}.json')
                    
                    with open(mappings_file, 'w') as f:
                        json.dump(encoding_mappings, f)
                    
                    # Add file reference to cleaner_config
                    result['cleaner_config']["encoding_mappings_file"] = mappings_file
                    logger.info(f"Saved encoding mappings to {mappings_file}")
                except Exception as e:
                    logger.error(f"Error saving encoding mappings to file: {str(e)}")
            
            # Save cleaning prompt to database
        ...
        try:
            ...
            # Convert to format expected by feature selector
            ...
            result['selected_features'] = list(feature_result["transformed_data"][0].keys()) if feature_result["transformed_data"] else []
            result['feature_selector_config'] = feature_result.get("options", {})
            result['feature_importances'] = feature_result.get("feature_importances", {})
            
            # Convert selected features back to DataFrame
            X_selected = pd.DataFrame.from_dict(feature_result["transformed_data"])
            
            logger.info(f"Feature selection completed successfully. Selected {len(result['selected_features'])} features.")
            
        except Exception as e:
            logger.error(f"Error in feature selection step: {str(e)}")
        ...
        try:
            ...
            result['anomaly_results'] = {
                ...
                'anomaly_report': anomaly_results.get("anomaly_report", {})
            }
            
            logger.info("Anomaly detection completed successfully")
            
            # Check if data is valid based on anomaly detection
            if not result['anomaly_results']['is_data_valid']:
                logger.warning("Anomaly detector reports data as invalid. Proceeding with caution.")
            
        except Exception as e:
            logger.error(f"Error in anomaly detection step: {str(e)}")
            return jsonify({'error': f'Anomaly detection failed: {str(e)}'}), 500
            
        # --- STEP 4: MODEL TRAINING ---
        logger.info("STEP 4: Training models with preprocessed data")
        ...
        try:
            ...
            # Get available model services
            model_services = get_model_services()
            logger.info(f"Available model services: {list(model_services.keys())}")
            
            if not model_services:
                return jsonify({'error': 'No model services available for training'}), 503
                
            # Setup training results collection
            models_results = []
            errors = []
            ...
            for model_name, model_url in model_services.items():
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'model' in model_result and 'metrics' in model_result['model']:
                            metrics = model_result['model']['metrics']
                            logger.info(f"Successfully trained {model_name} model. Metrics: {metrics}")
                            
                            # Clean up metrics (convert to float)
                            clean_metrics = {}
                            for metric_name, metric_value in metrics.items():
                                try:
                                    if isinstance(metric_value, str):
                            ...
                            # Replace metrics with clean metrics
                            model_result['model']['metrics'] = clean_metrics
            ...
            if not models_results:
                all_errors = '; '.join(errors)
                logger.error(f"Failed to train any models. Errors: {all_errors}")
                return jsonify({'error': f'Failed to train any models: {all_errors}'}), 500
                
            # Find best models for each metric
            best_models = find_best_models(models_results)
            logger.info(f"Found {len(best_models)} best models by metric")
            
            # Save best models to blob storage and database
            saved_best_models = save_best_models(best_models, user_id, run_id)
            logger.info(f"Saved {len(saved_best_models)} best models to Azure Blob Storage")
            
            # Add model results to the result dictionary
            result['model_training_results'] = {
                'models': models_results,
                'best_models': best_models,
                'saved_best_models': saved_best_models,
                'errors': errors
        ...
        logger.error(f"ERROR in preprocess_and_train: {str(e)}")
        return jsonify({'error': str(e)}), 500
