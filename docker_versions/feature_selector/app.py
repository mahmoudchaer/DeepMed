...
# Add new endpoint for applying stored feature selection
@app.route('/apply_stored_feature_selection', methods=['POST'])
def apply_stored_feature_selection_endpoint():
    """Apply the stored feature selection configuration to a dataframe"""
    try:
        data = request.json
        if not data or 'data' not in data or 'feature_selector_config' not in data:
            return jsonify({"error": "Missing data or feature_selector_config in request"}), 400
        
        # Convert the data to a pandas DataFrame
        df = pd.DataFrame(data['data'])
        feature_selector_config = data['feature_selector_config']
        ...
        for col in df.columns:
            ...
            if any(pattern in col_lower for pattern in irrelevant_patterns):
                ...
                logging.info(f"Identified potentially irrelevant column in feature selection: '{col}'")
        ...
        if 'selected_columns' in feature_selector_config and feature_selector_config['selected_columns']:
            ...
            for col in available_columns:
                if col in columns_to_exclude:
                    logging.warning(f"⚠️ Feature selector included potentially irrelevant column '{col}' - excluding it.")
                else:
                    validated_columns.append(col)
            
            logging.info(f"✅ Using {len(validated_columns)} validated features from feature selector config")
            ...
            logging.info(f"⚠️ No feature selection config found. Excluded {len(columns_to_exclude)} irrelevant columns, keeping {len(remaining_columns)}.")
        ...
        if cleaned_df is None or cleaned_df.empty or len(cleaned_df.columns) == 0:
            logging.warning("⚠️ No columns left after feature selection! Falling back to original features.")
            ...
            if not fallback_columns:
                logging.error("❌ Critical error: No usable features found. Using original dataframe.")
                cleaned_df = df.copy()
            else:
                cleaned_df = df[fallback_columns].copy()
        
        # Log the final feature selection
        logging.info(f"✅ Feature selection complete. Selected {len(cleaned_df.columns)} features out of {len(original_df.columns)} original features.")
        
        response = {
            "data": json.loads(cleaned_df.to_json(orient='records')),
            "selected_columns": cleaned_df.columns.tolist(),
            "report": {
                "rows_before": len(df),
        ...
        logging.error(f"Error in apply_stored_feature_selection: {str(e)}", exc_info=True)
