...
EXPERIMENT_NAME = os.getenv('MLFLOW_EXPERIMENT_NAME', 'knn_model')
...
class KNNTrainer:
    def __init__(self, test_size=0.2):
        ...
        self.model_config = {
            ...
            'params': {
                'classifier__n_neighbors': [5, 11],  # Reduced from 4 to 2 options
                'classifier__weights': ['distance'],  # Fixed option
                'classifier__metric': ['euclidean']   # Fixed option
            },
    ...
    def train(self, X_train, X_test, y_train, y_test):
        """Train KNN model and track with MLflow"""
...
model_trainer = KNNTrainer()

@app.route('/health')
def health():
    return jsonify({
        "service": "knn_model_api",
        "status": "healthy"
    })
...
def download_model():
    ...
    try:
        ...
        return send_file(model_path, as_attachment=True, download_name='knn_model.joblib')
