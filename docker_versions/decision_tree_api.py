...
EXPERIMENT_NAME = os.getenv('MLFLOW_EXPERIMENT_NAME', 'decision_tree_model')
...
class DecisionTreeTrainer:
    def __init__(self, test_size=0.2):
        ...
        self.model_config = {
            ...
            'params': {
                'classifier__max_depth': [5, 10],  # Reduced options
                'classifier__min_samples_split': [5],  # Fixed value instead of multiple
                'classifier__min_samples_leaf': [2],   # Fixed value instead of multiple
    ...
    def train(self, X_train, X_test, y_train, y_test):
        """Train decision tree model and track with MLflow"""
...
model_trainer = DecisionTreeTrainer()

@app.route('/health')
def health():
    return jsonify({
        "service": "decision_tree_model_api",
        "status": "healthy"
    })
...
def download_model():
    ...
    try:
        ...
        return send_file(model_path, as_attachment=True, download_name='decision_tree_model.joblib')
