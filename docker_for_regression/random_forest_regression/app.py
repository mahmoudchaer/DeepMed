...
class RandomForestRegressionModel:
    ...
    def train(self, X_train, y_train, X_test, y_test):
        ...
        with mlflow.start_run(run_name=f"random_forest_regression_tuning_{experiment_id}") as parent_run:
            ...
            model_basename = f"random_forest_regression_{timestamp}_{unique_id}"
            ...
            if not model_url:
                model_url = f"/saved_models/random_forest_regression/{model_basename}.joblib"
...
def health():
    ...
    return jsonify({
        "status": "healthy", 
        "service": "random_forest_regression",
...
def download_model():
    ...
    try:
        ...
        for file in os.listdir(SAVED_MODELS_DIR):
            if file.startswith('random_forest_regression_') and file.endswith('.joblib'):
                file_path = os.path.join(SAVED_MODELS_DIR, file)
        ...
        response = send_file(
            model_path, 
            as_attachment=True, 
            download_name='random_forest_regression_model.joblib',
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting Random Forest Regression Model Service on port {port}")
