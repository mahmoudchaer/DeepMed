...
class KNNRegressionModel:
    ...
    def train(self, X_train, y_train, X_test, y_test):
        ...
        logger.info(f"Training KNN Regression model with {X_train.shape[1]} features")
    ...
    def predict(self, X):
        """Make predictions with the trained model and scaler"""
    ...
    def load_model(self, model_path):
        """Load a saved model pipeline (scaler + model)"""
        try:
            ...
            self.is_trained = True
            logger.info(f"Model pipeline loaded from {full_path}")
            return True
...
regression_model = KNNRegressionModel()
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting KNN Regression Model Service on port {port}")
