...
EXPERIMENT_NAME = os.getenv('MLFLOW_EXPERIMENT_NAME', 'ridge_regression_model')
...
class RidgeRegressionTrainer:
    def __init__(self, test_size=0.2):
        ...
        # Define model with hyperparameters for grid search
        self.model_config = {
            'model': Ridge(),
            'params': {
    ...
    def train(self, X_train, X_test, y_train, y_test):
        """Train ridge regression model and track with MLflow"""
        logger.info("Starting ridge regression model training")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Ridge Regression API on port {PORT}")
